defmodule SlenderChannel do
  ...
  Becomes:
      ...
      def handle_in("frequency", %{"fm" => true}, socket) do
        case Phoenix.Channel.broadcast_from socket, "frequency", %{"fm" => true} do
          :ok -> {:reply, :ok, socket}
          _ -> {:reply, :error, socket}
        end
      end
  ...
  the given event and payload, broadcasting them to all *other* clients. Raises if broadcast fails.
